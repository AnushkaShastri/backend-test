...
[[ -z "$accid" ]] && echo "Account ID is Required" && exit 1
...
docker tag $env-$repoName:$imageTag $accid.dkr.ecr.$region.amazonaws.com/$env-$repoName:$imageTag

docker push $accid.dkr.ecr.$region.amazonaws.com/$env-$repoName:$imageTag
